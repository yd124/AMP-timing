...
exclude_ids = c("10502", "40640", "41002","41136", "41146", "40814", "41074", "41086")
...
tmp_cens %>%
  mutate(cens = 1*(days < 0) )%>%
  select(ID, days, log10VL, cens) %>%
  subset(!ID %in% exclude_ids) %>%
  write_csv("../../data/RV217MonoCens_2.csv")
...
tmp %>%
  ...
  subset(!ID %in% exclude_ids) %>%
  write_csv("../../data/RV217Mono_2.csv")
...
tmp %>%
  ...
  write_csv("../../data/RV217MonoCells_2.csv")
...
raw_dat %>%
  ...
  mutate(
    ...
    log10VL_pred = if_else(predicted_VL, 
                           ...
                           if_else(is.na(log10VL) & days<0, 
                                   log10(15),
                                   log10VL)),
    log10VL_cens = if_else(is.na(log10VL) & days<0, 
                           log10(15),
                           log10VL),
    cens = 1*(days < 0),
    cens_pred = 1*(log10VL_pred <= log10(15))
    ) %>%
  group_by(ID) %>%
  mutate(
